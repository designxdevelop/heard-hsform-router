(function(){let M=window.location.search.includes("debug=true");function W(...q){if(M)console.log("[Webflow Scheduler]",...q)}let O={over_100k:{url:"https://meetings.hubspot.com/bz/consultation",name:"Consultation Scheduler",description:"Revenue over $100k"},under_100k:{url:"https://meetings.hubspot.com/bz/consultations",name:"Consultations Scheduler",description:"Revenue under $100k or not provided"}},L={portalId:window.HDYHAU_CONFIG&&window.HDYHAU_CONFIG.portalId||"7507639",formId:window.HDYHAU_CONFIG&&window.HDYHAU_CONFIG.formId||window.HDYHAU_FORM_ID||"2abfe31d-49b3-433d-9776-4ff663f8c0b9",sourcePropertyName:window.HDYHAU_CONFIG&&window.HDYHAU_CONFIG.sourcePropertyName||"how_did_you_heard_about_us",otherPropertyName:window.HDYHAU_CONFIG&&window.HDYHAU_CONFIG.otherPropertyName||"hdyhau_other_text"},F=[{label:"Search engine",value:"Search engine"},{label:"Social media",value:"Social media"},{label:"Reddit",value:"Reddit"},{label:"AI research",value:"AI research"},{label:"Podcast",value:"Podcast"},{label:"Blog, article, or news",value:"Blog, article, or news"},{label:"Influencer or content creator",value:"Influencer or content creator"},{label:"Postcard or mailer",value:"Postcard or mailer"},{label:"Professional association",value:"Professional association"},{label:"Therapy platform or tools",value:"Therapy platform or tools"},{label:"Friend, family, or colleague",value:"Friend, family, or colleague"},{label:"Billboard",value:"Billboard"}],N={label:"Other",value:"Other"},b=!1,U=!1;function w(){try{let q=sessionStorage.getItem("scheduler_router_data");if(q){let z=JSON.parse(q);return W("Found router data in sessionStorage:",z),sessionStorage.removeItem("scheduler_router_data"),{formData:z.formData,schedulerType:z.scheduler_type,source:"sessionStorage"}}}catch(q){W("sessionStorage error:",q)}try{let q=localStorage.getItem("hubspot_form_data");if(q){let z=JSON.parse(q);return W("Found form data in localStorage:",z),{formData:z,source:"localStorage"}}}catch(q){W("localStorage error:",q)}try{let q=(J)=>{let X=`; ${document.cookie}`.split(`; ${J}=`);if(X.length===2)return X.pop().split(";").shift()},z=q("scheduler_type"),G=q("form_data");if(z||G){let J=G?JSON.parse(atob(G)):{};return document.cookie="scheduler_type=; path=/; max-age=0",document.cookie="form_data=; path=/; max-age=0",W("Found data in cookies:",{schedulerType:z,formData:J}),{formData:J,schedulerType:z,source:"cookies"}}}catch(q){W("cookie error:",q)}return null}function B(){let q={},z=new URLSearchParams(window.location.search);for(let[G,J]of z)q[G]=J;return q}function C(q,z){if(q&&q.schedulerType)return q.schedulerType;if(q&&q.formData&&q.formData.scheduler_type)return q.formData.scheduler_type;if(z&&z.scheduler_type)return z.scheduler_type;return"under_100k"}function y(){try{let q=document.cookie.match(/(?:^|; )hubspotutk=([^;]*)/);return q?decodeURIComponent(q[1]):""}catch(q){return W("HubSpot cookie lookup error:",q),""}}function c(q){if(!q)return"";let z=["email","email_address","0-1/email","0-2/email"];for(let G of z)if(q[G])return String(q[G]).trim();return""}function h(q){let z=[...q];for(let G=z.length-1;G>0;G-=1){let J=Math.floor(Math.random()*(G+1));[z[G],z[J]]=[z[J],z[G]]}return[...z,N]}function I(q){return String(q).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function g(){if(!L.formId)return"";return"https://api.hsforms.com/submissions/v3/integration/submit/"+L.portalId+"/"+L.formId}function p(q,z,G){let J=g();if(!J)return Promise.reject(Error("Missing HDYHAU HubSpot form ID"));let $=[{name:"email",value:q},{name:L.sourcePropertyName,value:z}];if(G)$.push({name:L.otherPropertyName,value:G});let X={fields:$,context:{pageUri:window.location.href,pageName:document.title||"Schedule Confirmation"}},Z=y();if(Z)X.context.hutk=Z;return fetch(J,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(X),keepalive:!0}).then((Y)=>{if(!Y.ok)throw Error(`HubSpot HDYHAU submission failed: ${Y.status}`);return Y})}function u(){if(document.getElementById("hdyhau-styles"))return;let q=document.createElement("style");q.id="hdyhau-styles",q.textContent=`
    ...
    `,document.head.appendChild(q)}function P(q,z){if(b||U)return;let G=c(q);if(!G){W("Skipping HDYHAU form because no contact email was found");return}b=!0,u();let J=document.createElement("div");J.id="hdyhau-modal",J.className="hdyhau-modal",J.setAttribute("role","dialog"),J.setAttribute("aria-modal","true"),J.setAttribute("aria-labelledby","hdyhau-question-label");let $=document.createElement("div");$.className="hdyhau-dialog";let X=document.createElement("button");X.className="hdyhau-close",X.type="button",X.setAttribute("aria-label","Close"),X.textContent="×";let Z=document.createElement("form");Z.id="hdyhau-form",Z.className="hdyhau-form",Z.noValidate=!0;let Y=h(F).map((V,K)=>{let j=`hdyhau-option-${K}`;return`
